...
class WordEmbedding:
    def __init__(self, fname):
        ...
        if fname.endswith(".bin"):
            ...
            with open(fname, "r", encoding='utf8') as f:
                for line in f:
                    ...
                    s = line.split()
                    if len(vecs) and vecs[-1].shape[0]!=len(s)-1:
                        # print("Line num: ", count)
        ...
        # if max(norms)- min(norms) > 0.001:
        if maxNorm-minNorm > 0.0001:
            self.normalize()
    ...
    def reindex(self):
        ...
        print("self.n: ", self.n)
        print("self.word: ", len(self.words))
    ...
    def normalize(self):
        print("******* in the normalize function *******")
        self.desc += ", normalize"
        for vec in self.vecs:
